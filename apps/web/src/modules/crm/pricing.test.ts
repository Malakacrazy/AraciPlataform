...
describe("calcularTarifaHora", () => {
  ...
  it("a role with no salary configured still carries the studio's overhead", () => {
...
describe("calcularProposta", () => {
  ...
  it("falha alto se um papel com horas lançadas não tem RoleRate — não deve tratar como mão de obra grátis", () => {
    expect(() =>
      calcularProposta({
        roleHours: [
          { role: "Designer Pleno", stage: ProjectStageName.BRIEFING, hours: 5 },
        ],
        complexityScores: maxComplexity,
        contractedStages: [ProjectStageName.BRIEFING],
        roleRates: [], // nenhuma tarifa cadastrada para "Designer Pleno"
      })
    ).toThrow(/Designer Pleno/);
  });
});
